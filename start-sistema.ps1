...
# Ir siempre a la raíz del proyecto (carpeta del script)
cd "$PSScriptRoot"
...
Start-Process powershell -ArgumentList @(
  '-NoExit',
  '-Command',
  "cd `"$PWD`"; pnpm.cmd install --silent; pnpm.cmd start:dev"
)
...
Start-Process powershell -ArgumentList @(
  '-NoExit',
  '-Command',
  "cd `"$PWD`"; pnpm.cmd install --silent; pnpm.cmd dev"
)

Write-Host ""
Write-Host "✅ Todo en marcha!" -ForegroundColor Green
Write-Host "   - API:      http://localhost:3000/products"
Write-Host "   - Frontend: http://localhost:5174  (o 5173 si así lo configuraste)"
Write-Host "   - Adminer:  http://localhost:8080"

# (Opcional) Abrir URLs automáticamente:
